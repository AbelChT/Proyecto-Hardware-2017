...
void reversi8_placa()
{
	...
	int de=pulsado_derecha();
	int iz=pulsado_izquierda();
	while ( de==0 && iz==0){
		de=pulsado_derecha();
		iz=pulsado_izquierda();
	}
	...
	char __attribute__ ((aligned (8))) candidatas[DIM][DIM] =
	    ...
	    while (fin == 0)
	    {
	        ...
	        //esperar_mov(&ready_);
	        D8Led_symbol(15);
	        while (pulsado_izquierda()==0){ }
	        int fila_=0;
	        D8Led_symbol(fila_);
	        int se_ha_pulsado = 0;
	        resetCounter();
			while (pulsado_derecha()==0  ){ //|| se_ha_pulsado == 0
				if(pulsado_izquierda()==1)// se_ha_pulsado = 1;
				{
					fila_=get_contador();
					D8Led_symbol(fila_);
				}
			...
			while (pulsado_izquierda()==0){ }
			int columna_=0;
			D8Led_symbol(columna_);
			se_ha_pulsado = 0;
	        resetCounter();
			while (pulsado_derecha()==0  ){ // || se_ha_pulsado == 0
				if(pulsado_izquierda()==1) //se_ha_pulsado = 1;
				{
					columna_=get_contador();
					D8Led_symbol(columna_);
				}
	        ...
	        // si la fila o columna son 8 asumimos que el jugador no puede mover
	        if (((fila_) != DIM) && ((columna_) != DIM))
	        {
	            tablero[fila_][columna_] = FICHA_NEGRA;
	            actualizar_tablero(tablero, fila_, columna_, FICHA_NEGRA);
	            actualizar_candidatas(candidatas, fila_, columna_);
	            move = 1;
	        }
